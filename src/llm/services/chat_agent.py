"""
Enhanced Chat Agent Service - Simplified Command Handling

Simplified conversational AI that relies on intelligent query classification
rather than rigid regex pattern matching for better natural language understanding.
"""
...
class ChatAgent:
    """
    Enhanced chat agent with intelligent query routing that relies on natural language
    understanding rather than rigid command patterns.
    """
    ...
    def send_message(self, chat_session_id: str, user_message: str, 
        ...
        """
        Send a message and get intelligent response from chat agent.
        ...
        # Check for ONLY essential system commands (much simplified)
        special_response = self._handle_essential_commands(
        ...
        # For all other queries, use enhanced RAG with intelligent routing
        try:
    ...
    def _handle_essential_commands(self, message: str, chat_session_id: str, 
                                 analysis_session) -> Optional[Dict[str, Any]]:
        """
        Handle only essential system commands - much simplified from before.
        Most queries now flow to the RAG engine for intelligent processing.
        """
        ...
        if message_lower in ['summary', 'overview', 'what data do we have', 'data summary', 'status']:
        ...
        # Specific discussion exploration (with exact ID patterns)
        import re
        if re.match(r'^(explore|show|full)\s+(discussion|context|thread)\s+[a-z0-9]{6,}$', message_lower):
            parts = message.split()
            if len(parts) >= 3:
                content_id = parts[-1]
                content_type = 'post'
        ...
        # Let everything else flow to RAG engine for intelligent processing
        return None
    ...
    def list_chat_sessions(self, analysis_session_id: str) -> List[Dict[str, Any]]:
        ...
        for session in chat_sessions:
            ...
            preview = "New intelligent chat session"
            for message in reversed(messages):
...
Just ask naturally - I'll understand what you're looking for! 🚀💬"""
    ...
    def _format_enhanced_data_summary(self, summary: Dict[str, Any]) -> str:
        ...
        response += f"\n💬 Ask me questions naturally - I'll understand what you're looking for!"
        return response
    
    def _get_enhanced_help_message(self) -> str:
        """Get enhanced help message for natural language chat."""
        return """**💡 Intelligent Chat Help:**

**🗣️ Natural Language Questions:**
You can ask questions however feels natural! I understand:
• "What do people think about [topic]?"
• "How are people discussing [keyword]?"
• "Show me negative comments about [topic]"
• "Why is [keyword] mentioned so frequently?"
• "What are people's main complaints about [topic]?"
• "Tell me about discussions of [keyword]"

**🧠 I Automatically Handle:**
✅ **Smart Keyword Extraction** - I find the main topic from your question
✅ **Intent Understanding** - I know if you want stats, examples, or both
✅ **Graceful Fallbacks** - If a keyword isn't in your analysis, I'll still find discussions
✅ **Source Attribution** - Every answer includes where the information came from

**📊 System Commands:**
• `summary` - Overview of your analysis data
• `help` - This help message
• `explore discussion [ID]` - See complete Reddit thread

**🔍 How It Works:**
1. Ask any question about your data naturally
2. I extract the main topic and understand your intent
3. I search both analytics data and actual discussions
4. I provide comprehensive answers with real examples

**🎯 Tips for Best Results:**
• Include the main topic you're interested in
• Be specific about what you want to know
• Ask follow-up questions to dig deeper
• Use natural language - no special syntax needed

**Example Workflow:**
👤 "What can you tell me about how people discuss privacy concerns?"
🤖 I extract "privacy" as the topic, search your data, and show both analytics insights and real user discussions

Just ask naturally - I'll figure out what you're looking for! 🚀"""
